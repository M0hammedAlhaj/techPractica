...
@Component
@AllArgsConstructor
public class CreateRepoListener {
    private final MailSender mailSender;

    @EventListener
    public void handleJoinToSession(CreateRepoEvent event) throws MessagingException {
        log.info("CreateRepo received for user {}", event.userId());

        String emailReceiver = event.email();
        mailSender.sendMail(emailReceiver, "Your Repo Have Been Created!",
                createHtmlPage(event));

        log.info("Sent acceptance email to {}", event.email());
    }

    private static String createHtmlPage(CreateRepoEvent event) {
